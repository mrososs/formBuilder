...
@Component({
  ...
  template: `
    <button
      ...
      class="w-full p-3 border border-gray-200 hover:border-black hover:shadow-sm transition-shadow rounded-lg flex items-center gap-3 cursor-pointer"
    >
    ...
    <button
      ...
      (cdkDragStarted)="whileDragging.set(true)"
      (cdkDragEnded)="whileDragging.set(false)"
      class="w-full p-3 border border-gray-100 hover:border-black hover:shadow-sm transition-shadow rounded-lg flex items-center gap-3 cursor-pointer"
    >
      <div class="rounded-md bg-gray-100 flex items-center justify-center p-1">
        <mat-icon class="scale-75">{{ field().icon }}</mat-icon>
      </div>
